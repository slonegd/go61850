...
func TestParseReadResponse(t *testing.T) {
	tests := []struct {
		...
		{
			// Пакет из wireshark со структурой:
			...
			//         a2 18 - structure (Context-specific 2, Constructed, длина 24 байта)
			//            a2 07 - structure элемент (Context-specific 2, Constructed, длина 7 байт)
			//               87 05 08 3f 72 79 71 - floating-point значение (0.9471655488014221)
			//           84 03 03 00 00 - bit-string значение (padding 3, данные 0x0000, 13 бит)
			//           91 08 69 5b 9e d0 ab c6 a7 80 - utc-time значение (Jan 5, 2026 11:21:52.670999944 UTC)
			// Структура содержит 3 элемента: вложенную структуру (с float32), bit-string и utc-time
			name:   "structure с вложенной структурой, bit-string и utc-time",
			buffer: "a121020101a41ca11aa218a2078705083f72797184030300009108695b9ed0abc6a780",
			want: ReadResponse{
				InvokeID: 1,
				ListOfAccessResult: []AccessResult{{
					Success: true,
					Value: variant.NewStructureVariant([]*variant.Variant{
						// Вложенная структура с одним элементом - float32
						variant.NewStructureVariant([]*variant.Variant{
							variant.NewFloat32Variant(0.9471655488014221),
						}),
						// bit-string
						variant.NewBitStringVariant([]byte{0x00, 0x00}, 13),
						// utc-time
						variant.NewUTCTimeVariant(time.Date(2026, 1, 5, 11, 21, 52, 670999944, time.UTC)),
					}),
				}},
			},
		},
